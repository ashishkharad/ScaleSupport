...
/**
 * Reads all sheets in the spreadsheet and returns consolidated JSON
 */
function handleGetAllData(user, params) {
  ...
  // If accounts was not found under specific name, default to first sheet
...
// ==========================================
// BULK SYNCHRONIZATION (Site -> Sheet)
// ==========================================
...
function handleBulkSync(user, payload) {
  ...
  // Format A: Payload has explicit tabs with headers & rows
  ...
  // Format B: Payload has dictionary of sheets { 'Accounts': [ { ... } ] }
  ...
  try {
    ...
    for (var i = 1; i < rows.length; i++) {
      if (rows[i][0] == accountNumber || rows[i][1] == accountNumber) {
        // Update remark column if available
        if (rows[0].length >= 16) {
...
function rowToObject(headers, row) {
  ...
  for (var i = 0; i < headers.length; i++) {
    ...
    obj[cleanKey] = row[i];
    obj[rawKey] = row[i]; // Preserve original header name as well
  }
  return obj;
}
